 ...
 *
 * The panel is right-aligned to its anchor; on narrow screens (Slide Over, 320 px) it is shifted
 * right just enough to stay inside the viewport, and its width is capped to the viewport.
 */
export function Popover({ onClose, className = '', testId, children }: PopoverProps) {
  ...
  useLayoutEffect(() => {
    ...
    const fit = () => {
      el.style.transform = '';
      const overflowLeft = VIEWPORT_MARGIN - el.getBoundingClientRect().left;
  ...
  return (
    <div
      ref={ref}
      data-testid={testId}
      className={`absolute top-full right-0 z-30 mt-1.5 max-w-[calc(100vw-16px)] rounded-xl border border-ink-700 bg-ink-850 p-3 text-sm shadow-2xl shadow-black/50 ${className}`}
    >
      {children}
    </div>
  );
